...
Desktop notifications for [DeepSeek Harness (dsh)](https://github.com/deepseek-ai/deepseek-harness): get a system notification when a run finishes, and an immediate one when the model asks you a question (`ask_user_question`) or waits for approval (sandbox escalation / tool permission). The body reflects the result (completed / error / aborted / max-tokens / blocked / interrupted).

在运行结束、模型提问或等待审批时发送桌面通知。通知由 webview 客户端发出，打包时可选两种投递方案：浏览器原生 `Notification()` 或 Tauri 的 `@tauri-apps/plugin-notification`。
...
The install command runs `pnpm add` in the profile directory and appends packages that declare `dsh.bundle` to `dsh.profile.bundles`. After installing, **restart the web process** and hard-refresh the page.
